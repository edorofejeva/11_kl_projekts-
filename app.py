...
def top_data():
    try:
        ...
        return jsonify({'sttus': 'error'}), 500
    
@app.route('/pievieot-rezultatu', method= ['POST'])
def pievienot_rezultatu():
    dati = request.json
    try:
        pievienot(dati)
        top_rezultati = get_topresult()
        top_5 =sorted(top_rezultati, key =lambda x: (x['klikski'], x['laik']))[:5]
